...
public interface IPetRepo extends IGenericRepo<Pet, UUID>, JpaSpecificationExecutor<Pet> {
    ...
    @Query(value = "SELECT p.active, p.pet_age, p.pet_weight, p.created_at, p.deleted_at, p.updated_at, p.created_by, p.deleted_by, p.pet_id, p.shelter_id, p.updated_by, p.pet_breed, p.pet_name, p.pet_found_place, p.pet_temperament, p.pet_sex, p.pet_size, p.pet_state, p.pet_type\n" +
